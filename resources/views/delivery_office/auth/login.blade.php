...
@section('content')
  ...
  <div class='bl_login'>
    <div class='bl_login_inner'>
      <h2>依頼者ログイン</h2>
      <form method="POST" action="{{ route('delivery_office.login') }}">
        @csrf
      ...
      <div class="bl_login_inner_link">
        <div class="bl_login_inner_link_item">
          <a href="{{ route('delivery_office.register_request.create') }}">会員登録申請はこちら</a>
        </div>
        <div class="bl_login_inner_link_item">
          <a href="{{ route('delivery_office.password.request') }}">パスワードを忘れた場合はこちら</a>
        </div>
        <div class="bl_login_inner_link_item">
          <a href="{{ route('driver.login') }}">ドライバーの方はこちら</a>
        </div>
      </div>
    </div>
  </div>
