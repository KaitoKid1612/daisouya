      ...
      <div class="bl_reviewCreate_inner_content">
        @if ($task)
          <div class="bl_reviewCreate_inner_content_form">
            <form action="{{ route('delivery_office.driver_task_review.store') }}" method="POST" class="js_confirm">
              @csrf
              ...
              <section class="bl_reviewCreate_inner_content_form_request">
                <div class="bl_reviewCreate_inner_content_form_request_item">
                  <label for="driver">担当ドライバー</label>
                  <p>{{ $task->joinDriver->name_sei ?? 'ドライバーがいません' }} {{ $task->joinDriver->name_mei ?? '' }}</p>
                  <input type="hidden" name="driver_id" value="{{ $task->driver_id }}">
