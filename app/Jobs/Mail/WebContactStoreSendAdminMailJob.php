...
/**
 * お問合せメールのジョブ to Admin
 */
class WebContactStoreSendAdminMailJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            Mail::to($to_admin)->send(new WebContactStoreSendAdminMail($data_mail)); // 送信
            $msg = 'メールを送信しました。';
            $log_level = 7;
            $notice_type = 1;
        } catch (\Throwable $e) {
            ...
            WebNoticeLog::create([
                ...
                'to_user_info' => "{$config_base->site_name} / email:{$config_system->email_notice}",
                'user_id' => $login_id,
