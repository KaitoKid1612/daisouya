...
namespace App\Http\Controllers\Driver\Sanctum;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use App\Http\Requests\Driver\SanctumAuthTokenCreateRequest;
use App\Http\Requests\Driver\SanctumAuthTokenDestroyRequest;
    ...
    public function store(SanctumAuthTokenCreateRequest $request)
    {
        ...
        try {
            ...
            if (Hash::check($password, $user->password)) {
                return response()->json($user->createToken($device_name, ['driver'])->plainTextToken);
            } else {
                throw ValidationException::withMessages([
                    'email' => ['ログインできません'],
                ]);
