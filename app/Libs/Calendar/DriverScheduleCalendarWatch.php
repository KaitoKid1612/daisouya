 ...
 * カレンダー
 */
class DriverScheduleCalendarWatch extends BaseCalendar
{
  public $prev;
  public $next;
  ...
  public function __construct($calendar_month, $schedule_list, $task_list)
  {
    ...
    $this->schedule_list = $schedule_list; // ドライバーのスケジュール
    $this->task_list = $task_list; // ドライバーの稼働
    ...
    foreach ($period as $day) {
      ...
      foreach ($this->schedule_list as $item) {
        if ($day->format('Y-m-d') === $item->available_date) {
          ...
          $available_date_html = "<span data-action='{$route}' class='{$available_date_class} js_available_date el_absolute el_black' data-available_date = '{$item->available_date}'>&#9711;</span>";
          break;
        }
      }
      ...
      foreach ($this->task_list as $item) {
          $task_date_time = new \DateTime($item->task_date);
        if ($day->format('Y-m-d') === $task_date_time->format('Y-m-d')) {
          $task_date_class = 'task_date';
          $route = route('driver.driver_task.show', ['task_id' => $item->id]);
          $task_date_html = "<span href='{$route}' class='{$task_date_class} el_absolute'>&times;</span>";
          break;
        }
      }
      
      // 稼働があれば×。稼働がなく稼働日であれば○。
      $text = $task_date_html == '' ? $available_date_html : $task_date_html;
